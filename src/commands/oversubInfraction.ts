...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  // Retrieve the target user and attempt to resolve as a GuildMember.
  ...
  try {
    // Apply oversub punishment by adding 3 days.
    const newEnd = await subSuspension(targetUser.id);

    if (targetMember) {
      // If target is present in the guild, process suspension via RoleHandler.
      ...
      console.log(`[Oversub Command] Recorded suspension due for absent user ${targetUser.id}.`);
      await interaction.editReply(
        `<@${targetUser.id}> is not in the guild. Their oversub suspension has been recorded for processing when they rejoin.`
      );
    }
  } catch (error) {
    console.error('Error executing oversub command:', error);
