...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  try {
    // Update the suspension record by recording a moderate infraction.
    const result = await recordModerateInfraction(targetUser.id);

    if (targetMember) {
      if (result.tier < 6) {
        await RoleHandler.suspendMember(targetMember);
        const dmMessage = buildSuspensionNotice('moderate', result.tier, result.ends, reason, false);
        const channelMessage = buildSuspensionChannelMessage(targetUser.id, 'moderate', result.tier, result.ends, reason, false);
        try {
        ...
        const dmMessage = buildSuspensionNotice('moderate', result.tier, result.ends, reason, true);
        const channelMessage = buildSuspensionChannelMessage(targetUser.id, 'moderate', result.tier, result.ends, reason, true);
        try {
          await targetMember.user.send(dmMessage);
        } catch (err) {
          console.error(`Failed to DM <@${targetUser.id}>:`, err);
      ...
      // If the user is not in the guild, record a SuspensionDue document for later processing.
    ...
    console.error('Error executing moderate command:', error);
